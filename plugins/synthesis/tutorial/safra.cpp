...
class RabinGenerator : public vGenerator {
public:
    typedef std::pair<StateSet, StateSet> RabinPair;
    std::vector<RabinPair> mRabinPairs;

    RabinGenerator() : vGenerator() {}
    explicit RabinGenerator(const std::string& rFileName) : vGenerator() { Read(rFileName); }
    RabinGenerator(const RabinGenerator& rOther) : vGenerator(rOther), mRabinPairs(rOther.mRabinPairs) {}
    virtual ~RabinGenerator() {}

    void ClearRabinPairs() { mRabinPairs.clear(); }
    void InsRabinPair(const StateSet& rRSet, const StateSet& rISet) {
        mRabinPairs.push_back(std::make_pair(rRSet, rISet));
    }
    const std::vector<RabinPair>& RabinPairs() const { return mRabinPairs; }

    void Write(const std::string& rFileName) const { TokenWriter tw(rFileName); DoWrite(tw); }
    void Read(const std::string& rFileName) { TokenReader tr(rFileName); DoRead(tr); }
    void DWrite() const { TokenWriter tw(TokenWriter::Stdout); DoDWrite(tw, ""); }
        ...
        while (rTr.Peek(token)) {
            if (token.Type() == Token::Begin && token.StringValue() == "RabinPairs") {
                ...
                while (!rTr.Eos("RabinPairs")) {
                    ...
                    // Parse string like "[F1={S0 S2},E1={S3}], [F2=...,E2=...]"
                    size_t pos = 0;
...
public:
    ...
    SafraNode() : name(static_cast<Idx>(-1)), mark(false) {}
    explicit SafraNode(Idx n) : name(n), mark(false) {}
...
public:
    ...
    SafraTree() : root(static_cast<Idx>(-1)) {}
    explicit SafraTree(size_t buechi_size) : names(buechi_size * 2, false), root(0) {}
    SafraTree(const vGenerator& buechi) : names(buechi.Size() * 2, false) {
        root.name = 0;
        names[0] = true;
        root.mark = false;
...
// Helper functions adapted from safras.cpp
...
void SafraDet(const vGenerator& rGen, RabinGenerator& rRabinGen) {
    std::cout << "SafraDet(" << rGen.Name() << ") started" << std::endl;
    rRabinGen.Clear();
    rRabinGen.Name("SafraDet(" + rGen.Name() + ")");
    rRabinGen.InjectAlphabet(rGen.Alphabet());

    // Initialize
    SafraTree initialTree(rGen);
    std::cout << "Initial tree: " << initialTree.to_string(rGen) << std::endl;
    ...
    rRabinGen.StateName(initState, "S0"); // 明确命名为 S0
    todo.push(initialTree);

    size_t maxName = rGen.Size() * 2 - 1;
    std::vector<StateSet> acceptSets(maxName + 1);

    // Create states and transitions
    int stateCounter = 1; // 从 S1 开始编号后续状态
    while (!todo.empty()) {
        SafraTree currentTree = todo.top();
        todo.pop();
        Idx currentState = stateMap[currentTree];
        std::cout << "Processing state " << rRabinGen.StateName(currentState) << ": " << currentTree.to_string(rGen) << std::endl;

        for (EventSet::Iterator eit = rGen.Alphabet().Begin(); eit != rGen.Alphabet().End(); ++eit) {
            Idx event = *eit;
            SafraTree nextTree = ComputeTransition(rGen, currentTree, event);

            std::cout << "After transition " << rGen.EventName(event) << ": " << nextTree.to_string(rGen) << std::endl;
            ...
            if (it == stateMap.end()) {
                ...
                rRabinGen.StateName(nextState, "S" + std::to_string(stateCounter++)); // 依次命名为 S1, S2, ...
                todo.push(nextTree);
